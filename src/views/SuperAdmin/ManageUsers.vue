<template>
    ...
    <v-alert v-model="addSuccessSnackbarVisible" color="success">
      ...
      <v-card variant="outlined"  style="width: 90%; margin-top: 20px;margin-left: 20px;" elevation="4">
        ...
        <v-card-title style="background:#385cad;color:white">User's Table</v-card-title>
  ...
  <v-card >
    <v-card-title style="background: #385cad;color:white" class="text-center">Edit User</v-card-title>
    <v-card-text>
      <v-form ref="editForm" @submit.prevent="submitEditUser">
        ...
        <v-text-field v-model="editedUser.email" label="Email" required variant="outlined"></v-text-field>
        <v-select
          ...
          variant="outlined"
        ></v-select>
        <v-text-field v-model="editedUser.password" label="Password" :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'" @click:append="showPassword = !showPassword" :type="showPassword ? 'text' : 'password'" required variant="outlined"></v-text-field>
        <v-text-field v-model="confirmPassword" label="Confirm Password" :append-icon="showConfirmPassword ? 'mdi-eye' : 'mdi-eye-off'" @click:append="showConfirmPassword = !showConfirmPassword" :type="showConfirmPassword ? 'text' : 'password'" required variant="outlined"></v-text-field>
  ...
  <v-card>
    <v-card-title style="background: #385cad;color:white" class="text-center">Add User</v-card-title>
    <v-card-text>
      <v-form ref="addUserForm" >
        <v-text-field v-model="newUser.name" label="Name" required variant="outlined"></v-text-field>
        <v-text-field v-model="newUser.email" label="Email" required variant="outlined"></v-text-field>
        <v-select
          ...
          variant="outlined"
        ></v-select>
        <v-text-field v-model="newUser.password" label="Password" :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'" @click:append="showPassword = !showPassword" :type="showPassword ? 'text' : 'password'" required variant="outlined"></v-text-field>
        <v-text-field v-model="confirmPassword" label="Confirm Password" :append-icon="showConfirmPassword ? 'mdi-eye' : 'mdi-eye-off'" @click:append="showConfirmPassword = !showConfirmPassword" :type="showConfirmPassword ? 'text' : 'password'" required variant="outlined"></v-text-field>
    ...
    <v-card-actions>
      <v-btn color="white" @click="submitAddUser" style="background: green;text-transform: capitalize;margin-left: 10px;">Save</v-btn>
      <v-btn color="white" @click="cancelAddUser" style="background:red;text-transform: capitalize;">Cancel</v-btn>
    </v-card-actions>
  </v-card>
</v-dialog>
