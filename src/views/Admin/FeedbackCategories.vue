<template>
    ...
    <v-card variant="outlined" style="width: 90%; margin-top: 20px;margin-left: 20px;" elevation="4">
      ...
      <v-alert v-model="updateSuccessAlertVisible" type="success" dismissible style="margin-top: -20px;">
      Feedback category data updated successfully!
    </v-alert>

    <!-- Snackbar for adding service -->
    <v-alert v-model="addSuccessSnackbarVisible" color="success" style="margin-top: -20px;">
      Feedback category added successfully!
    </v-alert>

    <v-alert v-model="deleteSuccessAlertVisible" type="success" dismissible style="margin-top: -20px;">
...
export default {
  ...
  try {
    ...
    async deleteService() {
  try {
    // Make an API call to delete the feedback category
    const response = await axiosInstance.delete(`/feedback-categories/${this.selectedService.id}`);
    console.log('Feedback category deleted:', response.data);
