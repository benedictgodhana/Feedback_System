...
<v-col cols="12" md="2">
  ...
  <v-col cols="12" md="2" class="d-flex align-center">
    ...
    <v-btn color="green" style="text-transform: capitalize;margin-top: -36px;" class="mr-4" @click="exportFilteredFeedback">
      ...
      export default {
        ...
        props: {
    category: String // Define the category prop with the expected type
  },
        data() {
          return {
    ...
    filterSubCategory(subcategoryId) {
        ...
        try {
            const response = await axiosInstance.get(`/feedback-categories/${categoryId}/feedback_sub_categories`);
            this.subcategories = response.data; // Store fetched subcategories
        } catch (error) {
  ...
  try {
    ...
    const response = await axiosInstance.get(apiUrl, { params, responseType: 'blob' });
