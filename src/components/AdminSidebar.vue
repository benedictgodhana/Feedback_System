<template>
  <v-navigation-drawer v-model="sidebar" app class="sidebar-drawer hidden-sm-and-down" style="background-color:white;" elevation="4">
    <v-list nav>
      <!-- Dashboard -->
      <v-list-item @click="navigateTo('/admin')" prepend-icon="mdi-view-dashboard" title="Dashboard" color="white" elevation="0" active-class="active-button"></v-list-item>
      ...
      <v-list-group>
        ...
        <v-list-item @click="navigateTo('/admin/manage-feedback')" prepend-icon="mdi-email" title="All Feedback" color="white" elevation="0" active-class="active-button"></v-list-item>
  ...
  methods: {
    ...
    async fetchCategories() {
      try {
        ...
        this.categories = response.data; // Assuming response contains category objects with 'name' property
      } catch (error) {
